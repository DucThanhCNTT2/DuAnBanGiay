    ...
    <div class="row">
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td >{{ ($ndtsanphams->currentPage() - 1) * $ndtsanphams->perPage() + $loop->index + 1 }}</td>
                    <td>{{ $item->ndtMaSanPham }}</td>
                    <td>{{ $item->ndtTenSanPham }}</td>
                    <td class="text-center" style="height: 100px;">
                        <img src="{{ asset('storage/' . $item->ndtHinhAnh) }}" alt="Sản phẩm {{ $item->ndtMaSanPham }}" class="img-fluid" style="max-height: 80px;">
                    </td>
        ...
        <div class="d-flex justify-content-center mt-3">
            {{ $ndtsanphams->links('pagination::bootstrap-5') }}
        </div>
